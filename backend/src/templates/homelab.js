...
export const homelab = {
  id: 'homelab',
  label: 'Homelab',
  description: 'Internet -> Cloudflare -> Router -> Virt-Host -> NAS, mit zwei Detailebenen.',
  icon: 'home',
  color: '#38bdf8',
  ...
  build({ root, view, node, edge }) {
    const l2 = view({
      name: 'Virt-Host (intern)',
      description: 'Dienste und interne Verbindungen auf dem Virtualisierungs-Host.',
    });
    const l3 = view({
      name: 'nginx Routing',
      parentId: l2.id,
    ...
    node({
      id: 'host', name: 'Virtualisierungs-Host', category: 'hypervisor', status: 'active',
    ...
    edge('router', 'host', { kind: 'http', label: 'Portfreigabe :443' });
    ...
    inZone({ id: 'nginx', name: 'nginx Proxy', category: 'reverse-proxy', status: 'active', linkedViewId: l3.id, position: { x: 40, y: 70 }, fields: { ip: '192.168.1.30' }, notes: '**Doppelklick** zeigt das Routing.' });
