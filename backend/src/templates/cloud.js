...
export const cloud = {
  id: 'cloud',
  label: 'Cloud-Umgebung',
  description: 'Region, VPC, Load Balancer, Managed Services und Object Storage.',
  icon: 'cloud',
  color: '#fb923c',
  ...
  build({ view, node, edge }) {
    const l2 = view({
      ...
      description: 'Was innerhalb der privaten Subnetze läuft.',
    });
    ...
    inVpc({ id: 'alb', name: 'Load Balancer', category: 'load-balancer', position: { x: 40, y: 60 }, fields: { ...region, cost: '25 €' } });
    inVpc({ id: 'app', name: 'App-Service', category: 'managed-service', linkedViewId: l2.id, position: { x: 320, y: 60 }, fields: { ...region, cost: '180 €' }, notes: '**Doppelklick** zeigt die Anwendungsschicht.' });
    inVpc({ id: 'fn', name: 'Bildverarbeitung', category: 'serverless', position: { x: 600, y: 60 }, fields: { ...region, cost: '12 €' } });
    inVpc({ id: 'db', name: 'Managed PostgreSQL', category: 'managed-service', position: { x: 320, y: 200 }, fields: { ...region, platform: 'AWS RDS', cost: '210 €', criticality: 'Kritisch' } });
    inVpc({ id: 's3', name: 'Object Storage', category: 'object-storage', position: { x: 600, y: 200 }, fields: { ...region, cost: '35 €' } });

    node({ id: 'secrets', name: 'Secrets Manager', category: 'secrets', status: 'active', position: { x: 700, y: 150 }, fields: { ...region } });
    node({ id: 'logs', name: 'Logs & Metriken', category: 'monitoring', status: 'active', position: { x: 60, y: 150 }, fields: { ...region, sla: '99,9 %' } });
    ...
    edge('s3', 'fn', { kind: 'data-flow', label: 'Objekt erstellt', animated: true });
    edge('app', 'secrets', { kind: 'dependency', label: 'DB-Zugangsdaten', lineStyle: 'dashed' });
    edge('app', 'logs', { kind: 'monitoring', label: 'Metriken', lineStyle: 'dashed' });
    ...
    node({ id: 'svc-order', name: 'Bestellungen', category: 'web-app', status: 'active', viewId: l2.id, position: { x: 140, y: 240 } });
    node({ id: 'svc-user', name: 'Benutzerverwaltung', category: 'web-app', status: 'active', viewId: l2.id, position: { x: 400, y: 240 } });
    node({ id: 'svc-bill', name: 'Abrechnung', category: 'web-app', status: 'planned', viewId: l2.id, position: { x: 660, y: 240 } });
