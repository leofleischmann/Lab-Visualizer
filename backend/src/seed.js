...
export function seedExampleForUser(db, userId) {
  ...
  node({ id: nid('internet'), name: 'Internet', category: 'internet', status: 'active', viewId: root.id, position: { x: 420, y: 40 } });
  node({ id: nid('domain'), name: 'example.com', category: 'domain', status: 'active', viewId: root.id, position: { x: 120, y: 220 } });
  node({ id: nid('cloudflare'), name: 'Cloudflare', category: 'cloud-service', status: 'active', viewId: root.id, position: { x: 420, y: 220 }, fields: { ip: '104.16.0.1' } });
  node({ id: nid('router'), name: 'OPNsense Router', category: 'router', status: 'active', viewId: root.id, position: { x: 420, y: 400 }, fields: { ip: '192.168.1.1', platform: 'OPNsense' } });
  node({
    id: nid('proxmox'), name: 'Proxmox Host', category: 'hypervisor', status: 'active', viewId: root.id,
    ...
    fields: { ip: '192.168.1.10', hostname: 'pve.lan', platform: 'Proxmox VE', os: 'Debian 12' },
    notes: '**Doppelklick** öffnet die Detailebene mit den internen Diensten.',
  });
  node({ id: nid('nas'), name: 'TrueNAS', category: 'storage', status: 'active', viewId: root.id, position: { x: 660, y: 600 }, fields: { ip: '192.168.1.20', platform: 'TrueNAS SCALE' } });
  ...
  inZone({ id: nid('nginx'), name: 'nginx Proxy', category: 'reverse-proxy', status: 'active', linkedViewId: l3.id, position: { x: 40, y: 70 }, fields: { ip: '192.168.1.30' }, notes: '**Doppelklick** zeigt das Routing.' });
  inZone({ id: nid('authelia'), name: 'Authelia (SSO)', category: 'auth', status: 'active', position: { x: 40, y: 250 } });
  inZone({ id: nid('gitea'), name: 'Gitea', category: 'git-repo', status: 'active', position: { x: 380, y: 40 }, fields: { ip: '192.168.1.31' } });
  inZone({ id: nid('nextcloud'), name: 'Nextcloud', category: 'web-app', status: 'active', position: { x: 380, y: 190 }, fields: { ip: '192.168.1.32' } });
  inZone({ id: nid('grafana'), name: 'Grafana', category: 'monitoring', status: 'maintenance', position: { x: 380, y: 340 } });
  inZone({ id: nid('postgres'), name: 'PostgreSQL', category: 'database', status: 'active', position: { x: 720, y: 70 }, fields: { ip: '192.168.1.40', platform: 'PostgreSQL 16' } });
  inZone({ id: nid('redis'), name: 'Redis', category: 'database', status: 'active', position: { x: 720, y: 300 } });
  ...
  node({ id: nid('nginx3'), name: 'nginx', category: 'reverse-proxy', status: 'active', viewId: l3.id, position: { x: 360, y: 40 } });
  node({ id: nid('up-gitea'), name: 'Gitea', category: 'git-repo', status: 'active', viewId: l3.id, position: { x: 120, y: 280 }, notes: 'Upstream :3000' });
  node({ id: nid('up-cloud'), name: 'Nextcloud', category: 'web-app', status: 'active', viewId: l3.id, position: { x: 360, y: 280 }, notes: 'Upstream :8080' });
  node({ id: nid('up-graf'), name: 'Grafana', category: 'monitoring', status: 'active', viewId: l3.id, position: { x: 600, y: 280 }, notes: 'Upstream :3000' });
